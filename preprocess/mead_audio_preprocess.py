...
def wavs2npys(input_wav_dir, input_label_dir, output_dir):
    ...
    # list all txt file
    name_list = [fn.split('.')[0] for fn in os.listdir(input_wav_dir) if fn.split('.')[1] == 'wav']
    wav_path = [os.path.join(input_wav_dir, file + '.wav') for file in name_list]
    label_path = [os.path.join(input_label_dir, 'BS_' + file + '_bs.npy') for file in name_list]
    ...
    for id in tqdm(os.listdir(data_dir)):
        for emo in os.listdir('/home/ubuntu/data/{}/blendshape/front/'.format(id)):
            for level in os.listdir('/home/ubuntu/data/{}/blendshape/front/{}/'.format(id, emo)):
                input_wav_dir = '/home/ubuntu/data/{}/video/front/{}/{}'.format(id, emo, level)
                input_label_dir = '/home/ubuntu/data/{}/blendshape/front/{}/{}'.format(id, emo, level)
                output_dir  = '/home/ubuntu/data/{}/video/front/{}/{}'.format(id, emo, level)
                print('input wav dir', input_wav_dir)
                print('input_label_dir', input_wav_dir)
                wavs2npys(input_wav_dir, input_label_dir, output_dir)

    print("name_num:", name_num, "frame_num:", frame_num, "error:", error)