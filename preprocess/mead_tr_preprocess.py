...
def bs2npy(fp):
    ...
    output_data = [frame[0]["transforms"]for frame in file]
...
def bss2npys(data_dir, output_dir):
    ...
    for fn in file_paths:
        ...
        np.save(os.path.join(output_dir, 'TR_{}.npy'.format(fn.split('.')[0])), output_data)
        print(output_data.shape, 'npy array saved to ', os.path.join(output_dir, 'TR_{}.npy'.format(fn.split('.')[0])))
